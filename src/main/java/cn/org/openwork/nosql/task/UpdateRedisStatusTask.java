...
public class UpdateRedisStatusTask extends AbstractRedisService
{
    ...
    public void execute()
    {
        ...
        for (RedisServer redisServer : redisServers)
        {
            ...
            if (isConnRedis(host, port, password))
            {
                ...
                RedisStatusHistory redisStatusHistory = getRedisStatusHistory(info, redisServer.getId());
                // 将该节点的info数据存入MySQL
     ...
     * @throws
     */
    private RedisStatusHistory getRedisStatusHistory(String info, Long serverId)
    {
        ...
        redisStatusHistory.setKeyspace(getRedisInfoKeyValue(info, "db0").trim());
        ...
        redisStatusHistory.setServerId(serverId);
