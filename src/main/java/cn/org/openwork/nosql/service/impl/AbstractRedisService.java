...
public class AbstractRedisService
{
    ...
    protected String getRedisInfo(RedisServer redisServer)
    {
        ...
        int port = Integer.parseInt(redisServer.getPort());
     ...
     * @throws
     */
    protected boolean isConnRedis(String host, String port, String password)
    {
    ...
    protected boolean isConnRedisRetry(String host, String port, String password)
    {
        boolean isConn = false;
        String ping = getRedisStatus(host, port, password);
     ...
     * @throws
     */
    protected String getRedisStatus(String host, String port, String password)
    {
        String status = "";
        Jedis jedis = null;
        try
        {
            RedisUtil redisUtil = new RedisUtil(host, Integer.valueOf(port), password);
            jedis = redisUtil.getJedis();
            status = jedis.ping();
        }
        catch (Exception e)
